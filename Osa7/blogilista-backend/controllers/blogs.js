...
blogsRouter.post('/:id/comments', async (request, response, next) => { //Post
  const body = request.body
  try {
...
blogsRouter.get('/:id/comments', async (request, response, next) => { //Näytä yksittäinen
  const commentsToShow = await Comment.find({})
  response.json(commentsToShow)
})
